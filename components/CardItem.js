...
export default function CardItem({ item, index }) {
    ...
    const bookmark = () => {
        if (bookmk === 'bookmark-o') {
            ...
            database.ref(`orders/${auth.currentUser.uid}/favorites/`).push(item);
        }
        if (bookmk === 'bookmark') {
    ...
    return (
        <Card style={styles.cstyle}>
            <View style={{ overflow: "hidden", borderTopLeftRadius: 9, borderTopRightRadius: 9 }}>
                <ImageBackground
                    source={{
                        uri:
                            item.restprofile.image.url,
                    }}
                    style={{
                        width: '100%',
                        height: 180,
